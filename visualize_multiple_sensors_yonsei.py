...
#####################################################################################################################
# pygame 이용해 rgb 카메라 4개, depth 카메라 4개를 시각화 합니다.
# visualize_multiple_sensors.py 예제 참고했습니다.
# 카메라는 차례대로 전, 후, 우, 좌 순서입니다.
#####################################################################################################################
...
class SensorManager:
    ...
    def save_image(self, image):
        ...
        array = array[:, :, :3]
        array = array[:, :, ::-1]
        self.cam_image = array # image 추출용
...
def run_simulation(args, client):
    ...
    try:
        ...
        # cam_image 추출용, front, rear, right, left 순서
        rgb_cam_image = [None, None, None, None]
        depth_cam_image = [None, None, None, None]

        # Display Manager organize all the sensors an its display in a window
        # If can easily configure the grid and the total window size
        display_manager = DisplayManager(grid_size=[2, 4], window_size=[args.width, args.height])
        ...
        SensorManager(world, display_manager, 'DepthCamera', carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=180)), 
                      vehicle, {}, display_pos=[1, 1])
        SensorManager(world, display_manager, 'DepthCamera', carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=+90)), 
        ...
        while True:
            ...
            # image 추출 테스트
            for idx, cam in enumerate(display_manager.sensor_list):
                if idx < 4:
                    rgb_cam_image[idx] = cam.image_export()
                else:
                    depth_cam_image[idx-4] = cam.image_export()
